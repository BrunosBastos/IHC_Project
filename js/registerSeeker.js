...
mailChimp();
      ...
      // });
        ...
        $("#SignInButton").click(function () { 
          ...
          var phoneNumber = $("#SeekerRegisterPhoneNumber").val()
          var day = $("#SeekerRegisterDay").val()
          var month = $("#SeekerRegisterMonth").val()
          var year = $("#SeekerRegisterYear").val()
          if (!(email.length==0 & password.length==0 & name.length==0 & city.length==0)) {
            if (!(/^\d+$/.test(phoneNumber) & /^\d+$/.test(day) & /^\d+$/.test(month) & /^\d+$/.test(year))) {
              console.log("error2")
            } else {
              var valid = true
              ...
              if (valid) {
                seekers.push({"name" : name, "dataNasc" : day+"/"+month+"/"+year, "city":city, "address": address, "email" : email, "password":password, "phoneNumber" : phoneNumber, "description" : "", "type" : type})
